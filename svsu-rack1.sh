...
readarray -t services < <(run_over_ssh ls /opt/rdama/app/*.sh|grep -i app)
echo "select the service to activate:"
select svc in "${services[@]}"; do
echo "service selected is $svc"
#expect -c "spawn ssh $user@$host "sh /opt/rdama/app/app-$version-SERVICE.sh start";expect "assword:";send "$pw";interact"
run_over_ssh "sh $svc start"
 break
done
}
